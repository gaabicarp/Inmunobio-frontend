...
import { BlogExperimento, Blogs } from 'src/app/models/blogs.model';
...
export class NuevoBlogExperimentoComponent implements OnInit {
  ...
  crearBlog(): void{
    ...
    const nuevoBlog : BlogExperimento={
      id_experimento: this.idExperimento,
      blogs: blog
    }
    console.log(nuevoBlog)
    this.postService.crearBlogExperimento(nuevoBlog).subscribe(res => {
      if (res.Status === 'ok'){
        this.alert = true;
        this.estado = 'success';
